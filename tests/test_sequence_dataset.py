...
def test_notebook_block_dataset_preserves_sequence(tmp_path):
    from Bio import SeqIO
    from Bio.Seq import Seq
    from Bio.SeqRecord import SeqRecord
    from minerva.finetuning import build_block_dataset

    record = SeqRecord(Seq("ACGT" * 10), id="test", annotations={"molecule_type": "DNA"})
    path = tmp_path / "test.gb"
    SeqIO.write([record], path, "genbank")

    class Tokenizer:
        def __call__(self, texts, **kwargs):
            # Keep one id per base and one for each strand marker.
            ids = [[0 if c == "X" else 1 for c in text.replace("<+>", "X")]
                   for text in texts]
            return {"input_ids": ids, "attention_mask": [[1] * len(row) for row in ids]}

    dataset = build_block_dataset(str(path), Tokenizer(), block_size=12)
    rows = dataset["train"]["input_ids"]
    assert len(rows) > 1
    assert all(len(row) <= 12 for row in rows)
    assert sum(row.count(1) for row in rows) == 40
