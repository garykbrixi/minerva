...
class MinervaForMaskedLM(MinervaPreTrainedModel):
    def __init__(self, config: MinervaConfig):
        super().__init__(config)
    ...
    def _interaction_head_names(self, interaction_layers: int) -> Dict[str, str]:
        ...
        """
        if interaction_layers not in (2, 6):
            raise ValueError("interaction_layers must be 2 or 6")

        tasks = ("base_pairing", "protein", "repeat")
        suffix = "" if interaction_layers == 2 else "_l6"
